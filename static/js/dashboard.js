...
// ============================================================
// Bank Comparison for Desktop
// ============================================================

let desktopBankComparisonChart = null;

// Bank colors (matching mobile version)
const DESKTOP_BANK_COLORS = {
    'CBSL': '#0d6efd',
    'HNB': '#198754',
    'PB': '#fd7e14',
    'SAMPATH': '#dc3545'
...
function loadDesktopBankComparison() {
    const months = document.getElementById('desktopBankComparisonMonths').value || 3;

    // Show loading, hide error and chart
    document.getElementById('desktopBankComparisonLoading').style.display = '';
    document.getElementById('desktopBankComparisonError').style.display = 'none';
    document.getElementById('desktopBankComparisonChartContainer').style.display = 'none';
    ...
    fetch('/api/exchange-rate/trends/all?' + params)
        ...
        .then(data => {
            // Hide loading
            document.getElementById('desktopBankComparisonLoading').style.display = 'none';

            // Show chart container
            document.getElementById('desktopBankComparisonChartContainer').style.display = '';

            // Update summary cards
            updateDesktopBankSummaryCards(data);

            // Render chart
            renderDesktopBankComparisonChart(data.source_comparison || {});
        })
        .catch(error => {
            ...
            document.getElementById('desktopBankComparisonLoading').style.display = 'none';
            document.getElementById('desktopBankComparisonError').style.display = '';
            document.getElementById('desktopBankComparisonErrorMsg').textContent = error.message || 'Failed to load bank comparison data.';
        });
}

// Update summary cards
function updateDesktopBankSummaryCards(data) {
    ...
    Object.keys(sources).forEach(bankName => {
        ...
        if (bankData && bankData.length > 0) {
            ...
            if (latest.buy_rate) {
                const rate = latest.buy_rate;
                const date = latest.date || '';

                // Update specific bank cards
                if (bankName === 'HNB') {
                    hnbRate = rate.toFixed(2);
                    hnbDate = date;
                } else if (bankName === 'SAMPATH') {
                    sampathRate = rate.toFixed(2);
                    sampathDate = date;
                }

                // Track best buy rate
                if (bestBuyRate === null || rate < bestBuyRate) {
                    bestBuyRate = rate;
    ...
    // Update all cards
    document.getElementById('desktopBestBuyRate').textContent = bestBuyRate !== null ? bestBuyRate.toFixed(2) : '--';
    document.getElementById('desktopBestBuyBank').textContent = bestBuyBank;
    document.getElementById('desktopCbslRate').textContent = cbslRate;
    document.getElementById('desktopCbslDate').textContent = cbslDate;
    document.getElementById('desktopHnbRate').textContent = hnbRate;
    document.getElementById('desktopHnbDate').textContent = hnbDate;
    document.getElementById('desktopSampathRate').textContent = sampathRate;
    document.getElementById('desktopSampathDate').textContent = sampathDate;
}

// Render the bank comparison chart
function renderDesktopBankComparisonChart(sources) {
    const canvas = document.getElementById('desktopBankComparisonChart');
    if (!canvas) {
    ...
    if (desktopBankComparisonChart) {
        desktopBankComparisonChart.destroy();
        desktopBankComparisonChart = null;
    }
    ...
    const datasets = bankNames.map(bankName => {
        ...
        return {
            label: bankName,
            data: data,
            borderColor: DESKTOP_BANK_COLORS[bankName] || '#6c757d',
    ...
    // Create chart
    desktopBankComparisonChart = new Chart(ctx, {
        type: 'line',
        data: {
            labels: labels,
            datasets: datasets
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const monthSelector = document.getElementById('desktopBankComparisonMonths');
    if (monthSelector) {
        monthSelector.addEventListener('change', function() {
            loadDesktopBankComparison();
        });
    }
